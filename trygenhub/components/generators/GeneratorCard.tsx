...
/** Badge text that triggers the glowing "just added" treatment
 * instead of the plain badge pill. Kept distinct from the existing
 * "New" badge copy used elsewhere in the data layer, so this never
 * changes the look of any pre-existing badge. */
const GLOW_BADGE_TEXT = new Set(["NEW", "НОВОЕ"]);
...
export function GeneratorCard({
  ...
  const isGlowBadge = Boolean(badge && GLOW_BADGE_TEXT.has(badge));
        ...
        <span className="absolute right-4 top-4 z-10 flex items-center gap-2">
          {badge && (
            <span
              aria-hidden="true"
              className={cn(
                "rounded-full bg-[var(--primary)] px-2 py-0.5 text-xs font-medium text-[var(--primary-foreground)]",
                isGlowBadge && "badge-glow-new",
              )}
            >
              {badge}
            </span>
