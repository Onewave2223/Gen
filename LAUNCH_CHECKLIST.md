...
- [ ] `npm ci` successful — **not run**: no `package-lock.json` exists yet
      in this archive; run `npm install` once (with registry access) to
      generate and commit one, then use `npm ci` afterward
- [ ] `npm run lint` successful — **not run** (no `node_modules`)
